...
Inputs::Inputs(HAPI_NodeId nodeId) : myNodeId(nodeId) {}

Inputs::~Inputs()
{
    for (AssetInputVector::iterator iter = myAssetInputs.begin();
         iter != myAssetInputs.end(); iter++)
    {
        delete *iter;
        *iter = NULL;
    }
...
Inputs::compute(MDataBlock &dataBlock)
{
    ...
    MArrayDataHandle inputArrayHandle = dataBlock.outputArrayValue(
        AssetNode::input, &status);
...
void
Inputs::setNumInputs(int numInputs)
{
...
Input::addNameChangedCallback(MObject &node)
{
    ...
    while (true)
    {
        MCallbackId callback = MNodeMessage::addNameChangedCallback(
            curNode, nameChangedCallback, this, &status);
...
void
Input::nameChangedCallback(MObject &node, const MString &str, void *clientData)
{
...
void
Input::setInputName(HAPI_AttributeOwner owner, int count, const MPlug &plug)
{
    myGeoPlug             = plug;
...
void
Input::setUnlockNormals(bool unlockNormals)
{
    myUnlockNormals = unlockNormals;
}

void
Input::setMatPerFace(bool matPerFace)
{
    myMatPerFace = matPerFace;
}

void
Input::setAllowFacetSet(bool allowFacetSet)
{
    myAllowFacetSet = allowFacetSet;
}

void
Input::setPreserveScale(bool preserveScale)
{
...
void
Input::setInputComponents(MDataBlock &dataBlock,
                          const MPlug &geoPlug,
