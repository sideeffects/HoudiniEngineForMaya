...
OutputInstancerObject::update()
{
    ...
    if (mySopNodeInfo.totalCookCount > myLastSopCookCount)
    {
        ...
        hapiGetPointAttribute(
            mySopNodeInfo.id, 0, "instance", attrInfo, instanceAttrs);
        for (unsigned int i = 0; i < instanceAttrs.length(); i++)
        {
            MStringArray splitObjName;
            instanceAttrs[i].split('/', splitObjName);
            myInstancedObjectNames.append(
                splitObjName[splitObjName.length() - 1]);
        ...
        // get a list of unique instanced names, and compute the object indices
        // that would be passed to Maya instancer
...
MStatus
OutputInstancerObject::compute(const MTime &time,
                               const MPlug &plug,
                               MDataBlock &data,
                               MDataHandle &handle,
                               ...
                               const bool needToRecomputeOutputData)
{
    ...
    {
        MDataHandle instancerDataHandle =
            handle.child(AssetNode::outputInstancerData);
        MArrayDataHandle instancedObjectNamesHandle =
            handle.child(AssetNode::outputInstancedObjectNames);
        MArrayDataHandle houdiniInstanceAttributeHandle =
            handle.child(AssetNode::outputHoudiniInstanceAttribute);
        MArrayDataHandle houdiniNameAttributeHandle =
            handle.child(AssetNode::outputHoudiniNameAttribute);
        MArrayDataHandle instanceTransformHandle =
            handle.child(AssetNode::outputInstanceTransform);
        ...
        for (unsigned int j = 0; j < size; j++)
        {
            ...
            MVector r = MQuaternion(
                            it.rotationQuaternion[0], it.rotationQuaternion[1],
                            it.rotationQuaternion[2], it.rotationQuaternion[3])
                            .asEulerRotation()
                            .asVector();
            ...
            MDataHandle intanceAttributeHandle =
                houdiniInstanceAttributeHandle.outputValue();
            intanceAttributeHandle.set(myHoudiniInstanceAttribute[j]);

            CHECK_MSTATUS(houdiniNameAttributeHandle.jumpToArrayElement(j));
            MDataHandle nameAttributeHandle =
                houdiniNameAttributeHandle.outputValue();
            ...
            MDataHandle translateHandle =
                transformHandle.child(AssetNode::outputInstanceTranslate);
            MDataHandle rotateHandle =
                transformHandle.child(AssetNode::outputInstanceRotate);
            MDataHandle scaleHandle =
                transformHandle.child(AssetNode::outputInstanceScale);

            MDataHandle txHandle =
                translateHandle.child(AssetNode::outputInstanceTranslateX);
            txHandle.set(p.x);
            MDataHandle tyHandle =
                translateHandle.child(AssetNode::outputInstanceTranslateY);
            tyHandle.set(p.y);
            MDataHandle tzHandle =
                translateHandle.child(AssetNode::outputInstanceTranslateZ);
            tzHandle.set(p.z);

            MDataHandle rxHandle =
                rotateHandle.child(AssetNode::outputInstanceRotateX);
            rxHandle.set(r.x);
            MDataHandle ryHandle =
                rotateHandle.child(AssetNode::outputInstanceRotateY);
            ryHandle.set(r.y);
            MDataHandle rzHandle =
                rotateHandle.child(AssetNode::outputInstanceRotateZ);
            rzHandle.set(r.z);

            MDataHandle sxHandle =
                scaleHandle.child(AssetNode::outputInstanceScaleX);
            sxHandle.set(s.x);
            MDataHandle syHandle =
                scaleHandle.child(AssetNode::outputInstanceScaleY);
            syHandle.set(s.y);
            MDataHandle szHandle =
                scaleHandle.child(AssetNode::outputInstanceScaleZ);
            szHandle.set(s.z);
        }
        ...
        else
        {
            ...
            Util::resizeArrayDataHandle(
                instancedObjectNamesHandle, myUniqueInstObjNames.length());
