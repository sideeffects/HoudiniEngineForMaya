...
CreateAttrOperation::createEnumAttr(const HAPI_ParmInfo &parm)
{
    ...
    HAPI_GetParmChoiceLists(NULL, myNodeInfo.id, choiceInfos, parm.choiceIndex, parm.choiceCount);
...
SyncAttribute::doIt()
{
...

    {
        ...
        HAPI_GetParameters(NULL, nodeInfo.id, &parmInfos[0], 0, parmInfos.size());
